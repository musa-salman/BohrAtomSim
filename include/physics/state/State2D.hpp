...
struct State2DBase : State {
    State2DBase(const math::Vector2 &r0, const math::Vector2 &v0)
    ...
    SIM_INLINE void integrate_common(math::scalar delta_time, const Pot &m_du) {
        time += delta_time;
        r_dot += r_ddot * delta_time;
        r += r_dot * delta_time;
        psi += psi_dot * delta_time;
        psi -= math::TWO_PI * static_cast<math::scalar>(psi >= math::TWO_PI);

        EOM::update(*this, m_du);
    }

    math::scalar r, r_dot, r_ddot;
    math::scalar psi, psi_dot;
...
template <typename Pot> struct State2D<false, false, Pot> : State2DBase {
    ...
    State2D(math::Vector2 r0, math::Vector2 v0, Factory &&factory)
...
struct State2D<false, true, Pot> : State2DBase, Quantized {
    ...
    SIM_CONST SIM_INLINE const std::array<math::scalar, field_names.size()>
    to_array() const noexcept {
...
struct State2D<true, false, Pot> : State2DBase, Relativistic {
    ...
    bool iterate(math::scalar delta_time) noexcept {
        prev_r_dot = r_dot;
        integrate_common<eom::polar::rel::EOM>(delta_time, m_du);
        return eom::isLocalMax(prev_r_dot, r_dot);
    }

    const std::array<math::scalar, field_names.size()>
    to_array() const noexcept {
