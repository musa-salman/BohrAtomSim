...
bool simulate_orbit_rel_step(struct sim_ctx *ctx, long double curr_l,
                             ...
                             long double *prev_max_vec, FILE *res_f);
...
bool simulate_orbit_rel_step(struct sim_ctx *ctx, long double curr_l,
    ...
    *is_maximum = !(*is_maximum);
    if (*is_maximum) {
        long double chi =
            calc_rel_chi(config->electron_mass, config->electron_charge,
                         ctx->electron_orbit->angular);
    ...
    next_itr->gamma = calc_rel_gamma(curr_l, config->electron_mass, R(curr_itr),
                                     R_DOT(curr_itr));
...
void simulate_orbit_rel(struct sim_ctx *ctx) {
    ...
    curr_itr->gamma = calc_rel_gamma(curr_l, config->electron_mass, curr_itr->r,
                                     curr_itr->r_dot);

    curr_itr->r_dot_dot = calc_rel_r_dot_dot(
        K_sqr * H_BAR_SQR, config->electron_mass, curr_itr->gamma, curr_itr->r,
        config->electron_charge, curr_itr->r_dot);

    curr_itr->phi_dot = calc_rel_phi_dot(curr_l, curr_itr->gamma, curr_itr->r,
                                         config->electron_mass);
