...
void simulate_spin_orbit(const struct sim_ctx ctx) {
    ...
    prev_itr.gamma =
        compute_spin_gamma(prev_itr.r, prev_itr.r_dot, prev_itr.theta,
                           prev_itr.theta_dot, orbit.angular);
    ...
    while (revolutions > 0) {
        ...
        iter_ctx.next_itr->gamma =
            compute_spin_gamma(RHO(iter_ctx.next_itr), R_DOT(iter_ctx.next_itr),
                               THETA(iter_ctx.next_itr),
